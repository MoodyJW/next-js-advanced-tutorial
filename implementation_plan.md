...
- **Files to create/modify:**
  - `app/layout.tsx`: We already use `next/font` for `Inter`. We will add `Fira_Code` specifically for code snippets to make it look premium.
  - `app/lessons/[slug]/page.tsx`: Export `generateMetadata` function. This fetches the lesson title and description from the database before rendering to populate the `<head>` tags for SEO.
  - [NEW] `components/LessonImage.tsx`: A reusable wrapper around `next/image`. Since our database doesn't have an `image_url` field, we will generate a high-quality "Next.js Mastery" banner image and serve it statically with built-in lazy loading and blur placeholders.
- **Key Code Patterns:** `export async function generateMetadata()`, `<Image />`, `Fira_Code({ subsets: ['latin'] })`.
- **TS Patterns:** `Metadata` and `ResolvingMetadata` types from `next`.

## User Review Required
> [!IMPORTANT]
> I will use my AI Image Generation tool to generate a stunning, premium banner image for the lesson pages. I'll save it to the `public/` directory so we can optimize it with `next/image`. Does that sound good?
...
- Inject the font CSS variable into the `<body>` so we can use `font-mono` securely.

#### [NEW] components/LessonImage.tsx
- Create a client/server component utilizing `next/image` to render a responsive, optimized image with a blur placeholder.

### Pages
#### [MODIFY] app/lessons/[slug]/page.tsx
- Add `export async function generateMetadata({ params }): Promise<Metadata>` to dynamically pull the lesson title and inject it into the page title and OpenGraph tags.
- Import and display `<LessonImage />` at the top of the lesson article.

## Verification Plan
### Automated Tests
- Add a test in `app/lessons/[slug]/page.test.tsx` to verify `generateMetadata` resolves correctly.
- Add a test for `LessonImage.tsx`.

### Manual Verification
- Inspect the `<head>` of a lesson page to ensure the `<title>` matches the database title.
- Inspect the Network tab to ensure `_next/image` is returning a smaller, WebP version of our generated banner.

### Phase 10: Error Handling
- **Objective:** Implement graceful error recovery.
- **Feature:** Handling deliberate runtime errors and network failures so the application does not crash completely, providing a good UX even during failure states.
- **Files to create/modify:**
  - [NEW] `app/error.tsx` & `app/error.test.tsx`: The global error boundary. This will catch any unhandled exceptions in the application and present a styled "Something went wrong" page with a recovery button.
  - [NEW] `app/lessons/[slug]/error.tsx`: The localized error boundary for lesson pages. If the database fails while fetching a specific lesson, only the lesson content area will crash, leaving the header and navigation intact.
- **Key Code Patterns:** Client component error boundaries (`"use client"`), `reset()` function to attempt recovery by re-rendering the segment.
- **TS Patterns:** Typing the `error` prop as `Error & { digest?: string }`.

## User Review Required
> [!IMPORTANT]
> The error boundaries in Next.js MUST be Client Components (`"use client"`). This is because errors can occur during hydration on the client or during rendering on the server, and a client component can handle both. Are you ready for me to create these error handling boundaries?

## Proposed Changes

### Global Error Boundary
#### [NEW] app/error.tsx
- Create a global error boundary for the application.
- Export a component receiving `{ error, reset }` props.
- Display a user-friendly error message and a "Try again" button that calls `reset()`.

#### [NEW] app/error.test.tsx
- Add a unit test verifying the component renders the error message and the reset button triggers the provided callback.

### Lesson-Specific Error Boundary
#### [NEW] app/lessons/[slug]/error.tsx
- Create a localized error boundary for the dynamic lesson pages.
- Style it to fit neatly inside the main content area (so the global layout and header remain visible).
- Provide a button to attempt reloading the lesson data.

## Verification Plan
### Automated Tests
- Run the newly created unit tests for both error components to ensure they render correctly and handle `reset()` clicks.
- Ensure the full test suite remains green.

### Manual Verification
- We can manually trigger an error during development by throwing a simulated error inside `app/lessons/[slug]/page.tsx` and verifying that only the lesson content area is replaced by the error boundary, not the entire page.
